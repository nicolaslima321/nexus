    ...
    <div className="flex flex-col items-center">
      <ol className="flex justify-center items-center p-3 space-x-2 text-sm font-medium text-center text-gray-500 bg-white border border-gray-200 rounded-lg shadow-sm dark:text-gray-400 sm:text-base dark:bg-gray-800 dark:border-gray-700 sm:p-4 sm:space-x-4 rtl:space-x-reverse w-10/12 sm:max-w-sm mb-6">
        <li className={`flex items-center ${liColorClass(1)}`}>
          <span
            className={`flex items-center justify-center w-5 h-5 me-2 text-xs border rounded-full shrink-0 ${spanColorClass(1)}`}
          >
            1
          </span>
        ...
        <li className={`flex items-center ${liColorClass(2)}`}>
          <span
            className={`flex items-center justify-center w-5 h-5 me-2 text-xs border rounded-full shrink-0 ${spanColorClass(2)}`}
          >
            2
          </span>
        ...
        <div className="p-4 sm:p-6 md:p-8">
          <div className="space-y-6">
            ...
            {step === 1 && (
              <SignupFirstStep
                survivor={survivor}
                onUpdateSurvivor={onUpdateSurvivorData}
              />
            )}
            ...
            {step === 2 && (
              <SignupSecondStep
                account={account}
                onUpdateAccount={onUpdateSurvivorData}
              />
            )}
