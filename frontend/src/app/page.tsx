...
export default function Home() {
  ...
  const reportsToRender: IReports[] = useMemo(() => {
    ...
    const { itemsPerSurvivorReport, statisticsAboutSurvivors } =
      generatedReport;

    const averageItemsReport = itemsPerSurvivorReport.map(
      ({ averageQuantityPerSurvivor, itemName }) => ({
    ...
    <div className="flex flex-col">
      <section className="self-center md:self-start mb-6">
        ...
        <p className="text-gray-700 dark:text-gray-400">
          Your camp has grown{" "}
          <Badge
            color={balanceOfCampGrown > 0 ? "green" : "red"}
            text={`${Math.round(balanceOfCampGrown)}%`}
          />{" "}
          this month
        </p>
      </section>
